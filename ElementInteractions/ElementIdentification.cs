...
namespace ElementInteractions
{
    ...
    public class ElementIdentification
    {
        ...
        public void ElementInterrogationTest()
        {
            ...
            //1. find button by Id
            var myElement1= driver.FindElement(By.Id("button1"));
            //2. GetAttribute("type") and assert that it equals the right value
            Assert.AreEqual("submit", myElement1.GetAttribute("type"));
            //3. GetCssValue("letter-spacing") and assert that it equals the correct value
            Assert.AreEqual("normal", myElement1.GetCssValue("letter-spacing"));
            //4. Assert that it's Displayed
            Assert.IsTrue(myElement1.Displayed);
            //5. Assert that it's Enabled
            Assert.IsTrue(myElement1.Enabled);
            //6. Assert that it's NOT selected
            Assert.IsFalse(myElement1.Selected);
            //7. Assert that the Text is correct
            Assert.AreEqual(myElement1.Text, "Click Me!");
            //8. Assert that the TagName is correct
            Assert.AreEqual("button", myElement1.TagName);
            //9. Assert that the size height is 21
            Assert.AreEqual(22, myElement1.Size.Height);
            //10. Assert that the location is x=190, y=330
            Assert.AreEqual(190, myElement1.Location.X);
            Assert.AreEqual(330, myElement1.Location.Y);
